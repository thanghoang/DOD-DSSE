...
int Server_DSSE::start()
{
    ...
    //load the data structure manually
    printf("Loading generated data structure....");
    DSSE* dsse = new DSSE();
    dsse->loadWhole_encrypted_matrix_from_file(this->I,this->serverID);
    delete dsse;
    printf("OK!\n");
#endif
    do
...
int Server_DSSE::updateBlock_data(zmq::socket_t& socket)
{
	...
	//socket.recv(this->serialized_data,this->serialized_data_len,0);
    // Send the ACK
    socket.send((unsigned char*)CMD_SUCCESS,sizeof(CMD_SUCCESS));
...
int Server_DSSE::getBlock_data(zmq::socket_t & socket)
{
    ...
    for(int i = 0 ; i < NUM_IDX_PER_DIM; i++)
    {
        ...
        tmp_1 = new MatrixType[MATRIX_COL_SIZE];
        memset(tmp_1,0,MATRIX_COL_SIZE);
        dsse->getBlock(index_request.search_idx[i],SEARCH_OPERATION,this->I,tmp_1);
        
        auto end2 = time_now;
        cout<<"getBlock time: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end2 - start2).count()/1000000.0<<endl;
        start2 = time_now;
        memcpy(&this->serialized_data[n],tmp_1,MATRIX_COL_SIZE);
        n += MATRIX_COL_SIZE;
        end2 = time_now;
        cout<<"copyBlock time: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end2 - start2).count()/1000000.0<<endl;
        
         start2 = time_now;
        tmp_2 = new MatrixType[MATRIX_ROW_SIZE/BYTE_SIZE];
        memset(tmp_2,0,MATRIX_ROW_SIZE/BYTE_SIZE);
        dsse->getBlock(index_request.update_idx[i],UPDATE_OPERATION,this->I,tmp_2);
        end2 = time_now;
        cout<<"getBlock time: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end2 - start2).count()/1000000.0<<endl;
        
        
        start2 = time_now;
        memcpy(&this->serialized_data[n],tmp_2,MATRIX_ROW_SIZE/BYTE_SIZE);
