/*
 * DynamicSSE_Hashmap_Key_Class.cpp
 *
...
hashmap_key_class::hashmap_key_class(const unsigned char *pIndata, int data_len){
	if ((data_len>0) && (pIndata != NULL)){
		this->pData = new unsigned char [data_len];
		this->data_length = data_len;
		//copy(pIndata,pIndata+data_len,this->pData);
...
hashmap_key_class::hashmap_key_class(const hashmap_key_class &rObj){
	if((rObj.pData != NULL) && (rObj.data_length>0)) {
		...
		//copy(rObj.pData,rObj.pData+rObj.data_length,this->pData);
...
//hashmap_key_class& hashmap_key_class::operator=(const unsigned char *pIndata){
//	if(pIndata != NULL){
//		copy(pIndata,pIndata+this->data_length,this->pData);
//		return *this;
//	}
//	else{
//		this->pData = NULL;
...
//
////equal comparison operator for this class
//bool hashmap_key_class::operator==(const hashmap_key_class &rObj) const {
//	if((this->data_length == 0) && (rObj.data_length ==0))
//		return 1;
//	else if((this->data_length == rObj.data_length)&& (memcmp(rObj.pData, this->pData,this->data_length ) == 0))
//		return 1;
//	else
//		return 0;
//}
...
////hashing operator for this class
//size_t hashmap_key_class::operator()() const{
//	size_t tmp = 0;
//	int maxhashsize = 0;
//
//	if (this->pData != NULL){
...
//			maxhashsize = sizeof(size_t);
//
//			tmp = tmp | this->pData[0];
//			for(indx=1; indx < maxhashsize; indx++){
//				tmp = tmp << 8;
//				tmp = tmp | this->pData[indx];
//			}
//			return tmp;
//		}
//	}
