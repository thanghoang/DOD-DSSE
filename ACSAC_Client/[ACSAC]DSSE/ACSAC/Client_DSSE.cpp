...
int Client_DSSE::createEncrypted_data_structure()
{
    ...
    * */
    ...
    try
    {
        ...
        // Allocating memory for operation
    ...
    for (int k = 0 ; k < NUM_SERVERS; k++)
    {
        ...
        memset(search_decrypted_b[k],0,MATRIX_COL_SIZE);
        ...
        memset (update_decrypted_b[k],0,MATRIX_ROW_SIZE/BYTE_SIZE);
        ...
        for(int i = 0 ; i < NUM_IDX_PER_DIM; i++)
        {
            ...
            memset(search_data[k][i],0,MATRIX_COL_SIZE);
            memset(update_data[k][i],0,MATRIX_ROW_SIZE/BYTE_SIZE);
            /*
...
int Client_DSSE::operation(int op, string strInput)        //CHANGE LATER PARAMETER
{
    ...
    cout<<"\n --- PREPARE TIME: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()/1000000.0<<endl ;
    /* 
   ...
   // pthread_attr_t attr;
    ...
    cout<<"\n --- PRECOMPUTE TIME: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()/1000000.0<<endl ;
    start = time_now;
    for(int k = 0 ; k < NUM_SERVERS; k++)
    {
        pthread_join(thread_requestData[k],NULL);
    ...
    cout<<"\n --- DOWNLOAD: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()/1000000.0<<endl ;
    auto end2 = time_now;
    cout<<"\n --- DOWNLOAD & PRECOMPUTATION: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end2 - start2).count()/1000000.0<<endl ;
    ...
    cout<<"\n --- TOTAL REENCRYPTION TIME: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()/1000000.0<<endl ;
    ...
    cout<<"\n --- TOTAL UPDATE BLOCK TIME: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()/1000000.0<<endl ;
#endif
    ret = 0;
...
int Client_DSSE::requestBlock_data(IndexRequest index_request, int serverID,MatrixType** search_data, MatrixType **update_data)
{
    ...
    {   
        ...
        //printf("OK\n");
        //auto end  = time_now;
        //cout<<"Time to say hello: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count()/1000000.0<<endl;
        ...
        message_t msg(this->serialized_data_len);
    
        socket.recv(&msg);
        cout<<"# bytes received: " <<msg.size()<<endl;
        
        //len=socket.recv(serialized_data,this->serialized_data_len,0);
        memcpy(this->serialized_data,msg.data(),this->serialized_data_len);
        
        //auto ttt2  = time_now;
        //cout<<"Time to download everything from socket: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(ttt2-ttt).count()/1000000.0<<endl;
        ...
        //end  = time_now;
        //cout<<"Time to download data from 1 server: "<<std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count()/1000000.0<<endl;
...
int Client_DSSE::sendBlock_data(IndexRequest index_request, int serverID, MatrixType** search_data, MatrixType **update_data)
 {
    ...
    try
    {
        ...
        message_t msg(this->serialized_data_len);
        memcpy(msg.data(),this->serialized_data,this->serialized_data_len);
        socket.send(msg,0);
        socket.recv(buffer_in,SOCKET_BUFFER_SIZE,0);
        //end  = time_now;
        //cout<<"Time to upload data to 1 server: "<< std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count()/1000000.0<<endl;
    }
    catch (exception &ex)
    {
